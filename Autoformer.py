...
# Load predicted probabilities for Autoformer
probs = np.load('./results/Autoformer/Autoformer_probs.npy')  # adjust path if needed
...
plt.title("Autoformer Anomaly Detection on Price Data")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig(os.path.join(save_dir, "autoformer_price_with_anomalies.png"))
plt.close()
