...
# Load predicted probabilities for TimesNet
probs = np.load('./results/TimesNet/TimesNet_probs.npy')  # adjust path if needed
...
plt.title("TimesNet Anomaly Detection on Price Data")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig(os.path.join(save_dir, "timesnet_price_with_anomalies.png"))
plt.close()
