...
# Load predicted probabilities for iTransformer
probs = np.load('./results/iTransformer/iTransformer_probs.npy')  # adjust path if needed
...
plt.title("iTransformer Anomaly Detection on Price Data")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig(os.path.join(save_dir, "itransformer_price_with_anomalies.png"))
plt.close()
